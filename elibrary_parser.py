...
class Elibrary(object):
    ...
    def parse_article_links(self):
        ...
        def analyse_page(parser,page):
            ...
            for tr in trs:
                ...
                if td is not None:
                    self.articles_links.append(td.find('a').get('href'))
    ...
    def parse_article_id(self):#Получение списка публикаций по ссылкам
        ...
        for artcl in self.articles_links:
            ...
            for tbl in tables:
                ...
                if tmp:
                    if re.findall('БИБЛИОМЕТРИЧЕСКИЕ ПОКАЗАТЕЛИ:', str(tmp)):
                        ...
                        if re.search('eISSN',items[2]):
                            issn_eissn = items[2].split('<span style="margin-left:20px;"></span>')
                            ...
                            edition_info.update({'name': self.remove_htmk_tags(items[0])})
                            edition_info.update({'edition': self.remove_htmk_tags(items[1]).replace('Издательство: ','')})
                            edition_info.update({'issn': self.remove_htmk_tags(items[2]).replace('ISSN: ',"")})
                ...
                for item in tmp:
                    for d in str(item).split('<span style="margin-left:20px;">'):
                        if re.findall('Тип:', d):
                            ...
                            edition_info.update({'year': int(self.remove_htmk_tags(d).replace('Год издания: ', '').replace('Год:', ''))})
            ...
            self.articles.append({'elibrary_id': eLIBRARY_ID, 'doi': doi, 'authors': authors, 'title': title, 'annotation': annotation, 'edition_info': edition_info, 'biblio_idicators': biblio_pokazatel})
